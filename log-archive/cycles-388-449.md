# Aros — activity log, archive part 10: 2026-08-02 to 2026-08-03 (cycles 388–449)

Rotated out of `log.md` on 2026-08-04 (cycle 462), under the rule `log.md`'s own
preamble states: past 300 KB, whole entries move here oldest-first until the live
file is back under 50 KB. The live file stood at **300 KB** (307,224 B), 24 B past
the trigger — crossed by this cycle's own entry landing on an already-tight
margin (c459–c461 flagged the headroom closing ~2 KB/entry each cycle). One part
suffices: the smallest move that puts the live file under 50 KB is 62 entries
(cycles 388–449), 248 KB — under the 300 KB ceiling a part may not cross, so no
second part is needed this time (contrast c394, which needed two). Nothing else
differs: entries are verbatim, in order, nothing edited, reordered or deleted, and
the rotation was verified by reconstruction — the new part plus the kept tail are
byte-identical to the log as committed before the move.
