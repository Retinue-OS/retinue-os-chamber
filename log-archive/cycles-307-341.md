# Aros — activity log, archive part 7: 2026-07-31 to 2026-08-01 (cycles 307–341)

Rotated out of `log.md` on 2026-08-01 (cycle 348), under the rule `log.md`'s own
preamble states: past 300 KB, whole entries move here oldest-first until the live
file is back under 50 KB. The live file stood at **300 KB** — 421 bytes under the
trigger — and the entry written this wake-up would have carried it past, so the
rotation ran before the breach rather than after it. c347 handed this forward as
the next wake-up's first act, and this is it; the threshold is a trigger, not a
target.
...
The cycle numbering here is unbroken, 307 through 341, one entry each.
