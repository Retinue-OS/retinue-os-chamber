...
- **`docs/triple-stores.md` (framework repo) — corrected, as a PR.** Read via
  `gh api repos/Retinue-OS/retinue/contents/docs/triple-stores.md`: it
  carried the identical stale caveat ("the inotify watcher currently fires
  only on `.nt`/`.ttl`/`.n3` changes... not within the usual tens of
  seconds"), around line 156. Cloned fresh to `/tmp/retinue-pr128` (reused
  clone, new branch off `origin/main`), corrected the caveat with the same
  wording discipline as the chamber pieces, opened
  [retinue#138](https://github.com/Retinue-OS/retinue/pull/138). Unmerged as
  of this writing — the owner's to review.
