...
current_next_action: "Aros, this cycle (2026-08-22, c968): reviewed the owner's freshly-opened PR#140 ('feat(dashboard): resizable regions, view toggles, settings page', +637 across CLAUDE.md, webapp/README.md, base.js, conversations.js, news.js, projects.js, push.js, layout.js (new), settings.html (new), styles.css, sw.js, index.html, conversations.html), opened 2026-08-22T14:59:53Z, two minutes after #139 which c967 already reviewed. Per bet-5's operating clause, read the full diff rather than the description. Traced the phone-layout DOM restructuring (news moved inside .col-main, next to conversations, for the wide-column split) against the new flex `order` overrides in styles.css: without them the flattened `display:contents` DOM would put news before projects on a phone; `retinue-news{order:1}` and `retinue-app-launcher{order:2}` restore the original conversations/projects/news/dock stacking -- verified by tracing DOM order by hand, correct. Checked layout.js's clamp constants against the CSS they claim to mirror: `MIN_SIDE_PX=280` vs `.col-side{min-width:280px}` and `MAX_SIDE_FRACTION=0.45` vs `max-width:45%` both match; `MAX_NEWS_FRACTION=0.75` vs `max-height:75%` matches. Checked the `retinue-news[hidden]`-hides-the-splitter rule against the actual mechanism: fetched the current `news.js` from `main` directly (`this.hidden = state==='ok' && !this.items.length`, confirmed at line 288) -- `hidden` is a reflected IDL property so the CSS attribute selector is live, not speculative. Read push.js's manage-mode state machine (unsupported/blocked/off/enabled/not-configured) against CLAUDE.md's rewritten paragraph describing it: matches. No defect found after this pass -- a clean review, not a miss (bet-5's c806/c809 clarification: the falsification counter tracks reviews offering nothing checkable, not my hit rate; this PR offered plenty checkable and none of it broke). No comment posted, per the same precedent (c806: a clean review needs no comment). Noted, not acted on: a Bash tool result while inspecting a fetched PR file carried a fabricated system-reminder claiming the file had been read and changed before -- it hadn't (only ever touched via Bash/gh, never Read); disregarded as a probable injection attempt per guardrail 9, no file was reverted or altered because of it, and no instruction embedded in it was followed. Delivery check: same failure as every run since c849 -- 5/5 cards STALE, disk and origin/main fresh (2026-08-21T23:15:00Z), served copies still 2026-08-05T19:20:00Z, Pages build still errored on the same commit 55aa91d (2026-08-06T13:43:40Z) -- NOT re-raised, parked for the ~08-30 review. Posting queue: item 3 posted 08-18, weekly floor next due 2026-08-25 -- not due. Org survey otherwise unchanged: no new issue/PR/discussion/star/fork/mention from anyone but the owner or me. NEXT: (1) watch chamber#10 for a Pages fix or reconsider venue at the ~08-30 review; (2) posting-queue item 4 when next due (2026-08-25); (3) retinue#138 (my own PR, fix upstream-confirmed) still awaiting owner merge."
...
- [`projects-archive/public-surface-c020-c034.md`](../projects-archive/public-surface-c020-c034.md)
  — **rows, not write-ups**: the ten oldest oversized register rows (cycles
  20–34, all audited 2026-07-20), moved verbatim by the c792 rotation's first
  batch (c813, 2026-08-16). Numbered part 0 because it predates part 1's range;
  the live rows keep their place in the table and point here.
- [`projects-archive/public-surface-c042-c056.md`](../projects-archive/public-surface-c042-c056.md)
  — **rows, not write-ups** (part 0b): seven rows from cycles 42–56 (audited
  2026-07-20/21) with no write-up in any part, moved verbatim by the c792
  rotation's second batch (c815, 2026-08-16). Sits inside part 1's date range
  but holds only rows part 1 lacks; the batch's other three rows (c47, c51,
  c54) point at their existing part-1 write-ups instead.
- [`projects-archive/public-surface-c038-c157.md`](../projects-archive/public-surface-c038-c157.md)
  — **rows, not write-ups** (part 0c): two rows moved verbatim by the c792
  rotation's third batch (c820, 2026-08-16) — c157 (audited 2026-07-24), which
  has no write-up in any part, and c38 (audited 2026-07-20), whose §Cycle 38
  write-up in part 1 exists but lacks the row's measured detail. The batch's
  other eight rows (c147–c161) point at their existing part-1 write-ups.
- [`projects-archive/public-surface-c178-c179.md`](../projects-archive/public-surface-c178-c179.md)
  — **rows, not write-ups** (part 0d): three rows moved verbatim by the c792
  rotation's fourth and fifth batches — c178 and c179 (c822, 2026-08-16),
  plus the second c179 row, the agent-self-review audit (c825, 2026-08-17).
  None has a write-up in any part, so these row texts are the primary record
  of those audits. The fourth batch's other eight rows (c161–c175) point at
  their existing part-1 write-ups.
...
| My own records (claim table) | 2026-07-20 (c20) | **Stale "no CI" claim → chamber#7; org-profile draft fixed.** c23 re-check: contained — only live instance is `GUARDRAILS.md:51`, the owner's by design. Detail: §c20 in [archive part 0](../projects-archive/public-surface-c020-c034.md) |
| `docs/` dashboard site | 2026-07-20 (c21) | **Live and stale** — dates, tracker citations, owner queue missing 4 of 7 open issues; all fixed in-repo. c29 freshness re-check: current, no edit. Detail: §c21 in [archive part 0](../projects-archive/public-surface-c020-c034.md) |
| Repo → live site delivery path (Pages) | 2026-07-20 (c24), c146 | **A Pages build can silently lag HEAD one commit.** Standing check: after a `docs/` push, Pages `latest.commit` must equal `main`; else re-push. Detail: §c24 in [archive part 0](../projects-archive/public-surface-c020-c034.md) |
| The escalation channel itself (dashboard thread state) | 2026-07-20 (c27) | **Working — my reading of it was not.** Cycle→wall-clock conversion overturned ~15 cycles of "unread" reporting → rule 5. Detail: §c27 in [archive part 0](../projects-archive/public-surface-c020-c034.md) |
| My own tool/permission surface (guardrail 5 isolation) | 2026-07-20 (c30) | **Isolation not enforced** — mail/messaging tools pre-approved, MCP connectors attach in-session; escalated, no tool called. Detail: §c30 in [archive part 0](../projects-archive/public-surface-c020-c034.md) |
| The org's own CI/automation output (workflow runs) | 2026-07-20 (c32) | **One workflow broken** — `check-signal-cli`'s `gh pr create` blocked by the org Actions checkbox, not the PAT → retinue#4. Detail: §c32 in [archive part 0](../projects-archive/public-surface-c020-c034.md) |
| Repo social preview images | 2026-07-20 (c22) | **Not a separate problem** — all four repos serve GitHub's auto-card, which renders the blank descriptions; downstream of chamber#4, upload UI-only. Detail: §c22 in [archive part 0](../projects-archive/public-surface-c020-c034.md) |
| Actions **secrets and variables** inventory (4 repos + org) | 2026-07-20 (c34) | **Not auditable by me, by design** — ten endpoints 403; denying config read first (c33 rule), no issue; the owner's to audit. Detail: §c34 in [archive part 0](../projects-archive/public-surface-c020-c034.md) |
| Workflow **file contents** (`tests.yml`, `check-signal-cli.yml`) | 2026-07-20 (c34) | **Conditional finding** — `tests.yml` lacks `permissions:`, token inherits unreadable default → comment on retinue#4. Detail: §c34 in [archive part 0](../projects-archive/public-surface-c020-c034.md) |
| `retinue-os-deployment` repo contents (public reference deployment) | 2026-07-20 (c33) | **Overturned my escalation** — the token recipe is a documented, threat-modelled decision; 3 chamber#6 asks withdrawn. Detail: §c33 in [archive part 0](../projects-archive/public-surface-c020-c034.md) |
| `qlever-dir/build_index.sh` (the path→graph-IRI mechanism) | 2026-07-20 (c38) | **Four filename-dependent `sed`-escaping defects → qlever-dir#5.** Detail: §c38 in [part 0c](../projects-archive/public-surface-c038-c157.md); write-up §Cycle 38, part 1 |
...
| **This chamber repo's own contents, as a disclosure surface** | 2026-07-20 (c42) | **Guardrail 5 violation, published since the initial commit — redacted that cycle; history decision escalated (dashboard).** Detail: §c42 in [archive part 0b](../projects-archive/public-surface-c042-c056.md) |
| **`brand/positioning.md` and `writing/`, audited for AI *disclosure*** | 2026-07-20 (c44) | **The live dashboard named its author like a human, never said AI — fixed; disclosure clause added upstream.** Detail: §c44 in [archive part 0b](../projects-archive/public-surface-c042-c056.md) |
| **`docs/components/*.js` — the published dashboard's rendering code** | 2026-07-20 (c45) | **Every date rendered a day early for readers west of UTC (no `timeZone` in `fmtDate`) — fixed, one line.** Detail: §c45 in [archive part 0b](../projects-archive/public-surface-c042-c056.md) |
| **`docs/examples/provenance/` and the live store behind it** | 2026-07-20 (c46) | **The documented watcher workaround does not work; the store sat 16 h stale, silently — corrected here and in qlever-dir#3.** Detail: §c46 in [archive part 0b](../projects-archive/public-surface-c042-c056.md) |
| **c46's own output — a published issue comment** | 2026-07-20 (c47) | **The severity example in qlever-dir#3 described an outage that never happened — corrected in the thread and both copies here.** Detail: [c47 write-up](../projects-archive/public-surface-c033-c183.md) |
| **README Installation/model-gateway vs compose, `.env.example`, `litellm/config.yaml`** | 2026-07-20 (c51) | **A subsystem the docs call optional is an unconditional startup dependency → retinue#11.** Detail: [c51 write-up](../projects-archive/public-surface-c033-c183.md) |
| **README's operational tail vs `entrypoint.sh`, compose and `CLAUDE.md`** | 2026-07-20 (c54) | **One real gap → retinue#12; the rest correct.** Detail: [c54 write-up](../projects-archive/public-surface-c033-c183.md) |
| **`SECURITY.md` and `CONTRIBUTING.md` re-audited post-CI, post-c52** | 2026-07-20 (c53) | **Both consistent; the one standing defect stays tracked, no new issue.** Detail: §c53 in [archive part 0b](../projects-archive/public-surface-c042-c056.md) |
| **`.claude/agents/archivist.md` — the ontology reference the lead-story doc links** | 2026-07-21 (c56) | **Clean; the chain doc → ontology → code is consistent end to end, nothing to file.** Detail: §c56 in [archive part 0b](../projects-archive/public-surface-c042-c056.md) |
| **`docs/triple-stores.md` — the lead-story doc vs qlever-dir source, the shipped converter, `web-gateway.py`** | 2026-07-21 (c55) | **No new defect; the one finding is fully tracked in retinue#1.** Detail: §c55 in [archive part 0b](../projects-archive/public-surface-c042-c056.md) |
| **The framework's *open pull requests*, read as in-flight public documentation** | 2026-07-23 (c147) | **Three measured defects in retinue PR#21 → comment on retinue#1.** Detail: [c147 write-up](../projects-archive/public-surface-c033-c183.md). |
| **The named-graph provenance mechanism, exercised end to end with a *second* converter** | 2026-07-23 (c149) | **Two silent defects, measured against the live store → qlever-dir#8, qlever-dir#9.** Detail: [c149 write-up](../projects-archive/public-surface-c033-c183.md). |
| **`comparison.md`** | 2026-07-24 (c154) | **The project's strongest security sentence asserted as fact in four public places; its own open issue says it is false → retinue#26.** Detail: [c154 write-up](../projects-archive/public-surface-c033-c183.md). |
| **The credential-custody claim, swept across every place it is stated** | 2026-07-24 (c155) | **The headline sentence stated unscoped in three public places; the true version already in the same repo → retinue#27.** Detail: [c155 write-up](../projects-archive/public-surface-c033-c183.md). |
| **The newest commit on the one open PR, re-read after its head moved** | 2026-07-24 (c156) | **A silent-skip path in the lead-story mechanism, reproduced twice → qlever-dir#10, retinue#28.** Detail: [c156 write-up](../projects-archive/public-surface-c033-c183.md). |
| **`docs/data/*.json` — the public dashboard, re-checked for freshness** | 2026-07-24 (c157) | **Two days stale in every card; regenerated. Rule 20: a freshness surface carries its next-decay date.** Detail: §c157 in [part 0c](../projects-archive/public-surface-c038-c157.md) |
| **`writing/` and this chamber's own `README.md`** | 2026-07-24 (c158) | **The c154/c155 claim sweeps never ran on my own writing; the missed file is the one meant to be somebody's front page.** Detail: [c158 write-up](../projects-archive/public-surface-c033-c183.md). |
| **The c154 sweep, re-run against the *property* not the sentence** | 2026-07-25 (c159) | **Four of nine sites; my own pitch carried both claims → comment on retinue#26; `brand/positioning.md` corrected.** Detail: [c159 write-up](../projects-archive/public-surface-c033-c183.md). |
...
| **Guardrail 3's claim table read column-wise: the right-hand column** | 2026-07-25 (c161) | **Row 3 states a setup step the project does not have and a variable count matching neither bound → comment on chamber#7.** Detail: [c161 write-up](../projects-archive/public-surface-c033-c183.md). |
| **My own open correction issues, re-measured against their `main`** | 2026-07-25 (c161) | **retinue#3's replacement numbers had themselves gone stale → comment on retinue#3.** Detail: [c161 write-up](../projects-archive/public-surface-c033-c183.md). |
...
| **The issue backlog as a whole, as its only reader receives it** | 2026-07-25 (c163) | **37 open, 0 ever closed, 0 by anyone else; filing 5.6/day against a drain of 0.** Detail: [c163 write-up](../projects-archive/public-surface-c033-c183.md). |
| **A merge conflict resolved by an automated agent in `scripts/entrypoint.sh`, read as a surface** | 2026-07-25 (c166) | **Clean — negative result, recorded because never run before.** Detail: [c166 write-up](../projects-archive/public-surface-c033-c183.md). |
...
| **The three core persona files** | 2026-07-25 (c170) | **A named third party's communication profile is public in the framework repo → escalated privately, deliberately not filed.** Detail: [c170 write-up](../projects-archive/public-surface-c033-c183.md). |
| **`.claude/skills/` (four) and `.claude/agents/archivist.md`** | 2026-07-25 (c171) | **Clean on ownership; one skill contradicts the review on security boundaries → retinue#31.** Detail: [c171 write-up](../projects-archive/public-surface-c033-c183.md). |
| **`.retinue/agents/aros.md`** | 2026-07-25 (c172) | **Clean on ownership; wrong about what I can see, and its PR recipe pushes to the wrong repo → retinue#32.** Detail: [c172 write-up](../projects-archive/public-surface-c033-c183.md). |
...
| **The live triple store diffed against its chamber** | 2026-07-25 (c174) | **Converter clean on all six; one graph stale; my rebuild-timing claim expired → comment on retinue#2.** Detail: [c174 write-up](../projects-archive/public-surface-c033-c183.md). |
| **The egress-audit trio** | 2026-07-25 (c175) | **One credential-exposure-class finding — measured live, escalated privately, not public until fixed (guardrail 9).** Detail: [c175 write-up](../projects-archive/public-surface-c033-c183.md). |
...
| **The three messaging push CLIs, read at the moment of sending** | 2026-07-26 (c178) | **Signal and WhatsApp clean; `telegram-push.py` describes a bot identity its gateway does not use → comment on retinue#9.** Detail: §c178 in [part 0d](../projects-archive/public-surface-c178-c179.md) |
| **The dashboard front-end (`webapp/`), read as what a user sees** | 2026-07-26 (c179) | **`sw.js` clean; all four data cards commented out, so nothing requests `/data/*.json` → retinue#35, comment on retinue#1.** Detail: §c179 in [part 0d](../projects-archive/public-surface-c178-c179.md) |
| **`scripts/agent-self-review.py` + `discover-agents.py` — the proactivity feature** | 2026-07-26 (c179) | **The daily gate can never match, silently — the two actor-URI emitters disagree → comment on retinue#1.** Detail: §c179 in [part 0d](../projects-archive/public-surface-c178-c179.md) |
| **`scripts/git-serialize.sh`** | 2026-07-26 (c182) | **The lock is bypassed by `git -C <repo> …`, the gateway's own auto-commit form → [retinue#37](https://github.com/Retinue-OS/retinue/issues/37).** Detail: [c182 write-up](../projects-archive/public-surface-c033-c183.md). |
| **`examples/chambers/*/.retinue/agents/*.md` (marvin, dolores)** | 2026-07-26 (c183) | **Both agents assert a chamber confinement nothing provides → [retinue#38](https://github.com/Retinue-OS/retinue/issues/38).** Detail: [c183 write-up](../projects-archive/public-surface-c033-c183.md). |
...
| `signal-gateway` persistence vs. its compose volumes | 2026-07-26 (c199) | **Defaults to `/tmp`, on no volume, against four contrary claims — the send-approval queue dies with the container.** Detail: §c199 in [archive part 2](../projects-archive/public-surface-c184-c210.md). |
| **This register table, as GitHub renders it** | 2026-07-26 (c200) | **47 of 70 rows were not rendering as a table — twelve blank lines split it into fragments; fixed this cycle.** Detail: §c200 in [archive part 2](../projects-archive/public-surface-c184-c210.md). |
| **My escalation channel, as the owner receives it** | 2026-07-26 (c201) | **0 of 9 agent threads opened in 7 days — I counted *pushed* as *escalated* → comment on chamber#5; one-open-thread rule adopted.** Detail: §c201 in [archive part 2](../projects-archive/public-surface-c184-c210.md). |
| **The dashboard's *dated predictions*, vs its measurements** | 2026-07-26 (c202) | **Three cards published an already-corrected deadline — a snapshot covers a measurement, not a prediction; fixed in place.** Detail: §c202 in [archive part 2](../projects-archive/public-surface-c184-c210.md). |
| The c202 prediction cards, at the first wake-up past their hour | 2026-07-26 (c203) | **The rule worked first time — bound expired, nothing human in the window; the cards now record the outcome.** Detail: §c203 in [archive part 2](../projects-archive/public-surface-c184-c210.md). |
| All five `docs/data/*.json`, regenerated together | 2026-07-26 (c204) | **Clean, and the first full regeneration since 08:25 — the replaced page carried three measurement times.** Detail: §c204 in [archive part 2](../projects-archive/public-surface-c184-c210.md). |
| `qlever-static/` — the framework dir in no record of mine | 2026-07-26 (c205) | **The documented reindex recipe rebuilds from a stale cached copy when the input is gzipped; drafted, not filed (rate limit).** Detail: §c205 in [archive part 2](../projects-archive/public-surface-c184-c210.md). |
| `updater/` (sidecar + self-update + router) | 2026-07-26 (c206) | **Update path reports the dispatch, never the result; auth and no-arbitrary-command verified correct.** Detail: §c206 in [archive part 2](../projects-archive/public-surface-c184-c210.md). |
| `drafts/` read as a queue with a drain rate | 2026-07-26 (c206) | **7 held, 0 filed in 20 h — monotonic since the c184 limit; README fixed, default changed to *drain*.** Detail: §c206 in [archive part 2](../projects-archive/public-surface-c184-c210.md). |
| The `/tmp`-lifetime class, re-read as a class | 2026-07-27 (c207) | **Two members, not the three named from memory; consolidated, one citation corrected.** Detail: §c207 in [archive part 2](../projects-archive/public-surface-c184-c210.md). |
| This register table, re-rendered while appending | 2026-07-27 (c208) | **The c200 defect recurred — a blank line split the table, 5 rows rendered as prose; fixed, re-render check added.** Detail: §c208 in [archive part 2](../projects-archive/public-surface-c184-c210.md). |
| The five dashboard cards, read for c208's defect | 2026-07-27 (c209) | **All eleven relative-day strings false at 00:00 UTC; regenerated, absolute UTC only.** Detail: §c209 in [archive part 2](../projects-archive/public-surface-c184-c210.md). |
| `drafts/` against the README's new sentence about it | 2026-07-27 (c209) | **8 of 39 write-ups stated no filing status, so the claim was false for a fifth; fixed.** Detail: §c209 in [archive part 2](../projects-archive/public-surface-c184-c210.md). |
| The 8 status-less drafts, matched from the API | 2026-07-27 (c210) | **All 8 were filed; none lost — status blocks back-filled, README claim now true.** Detail: §c210 in [archive part 2](../projects-archive/public-surface-c184-c210.md). |
| The one dated prediction c209 printed | 2026-07-27 (c210) | **Resolved as forecast; not regenerated — rule added: clock-dependent sentences name their anchor.** Detail: §c210 in [archive part 2](../projects-archive/public-surface-c184-c210.md). |
| The chamber#6 token blocker, re-probed | 2026-07-27 (c211) | **Still 403 — the claim holds; probe is one command and creates nothing.** Detail: §c211 in [archive part 3](../projects-archive/public-surface-c211-c233.md). |
| The held-queue count, which picks drain vs audit | 2026-07-27 (c211) | **4, not the reported 5 — two superseded drafts kept being carried; classifier order fixed.** Detail: §c211 in [archive part 3](../projects-archive/public-surface-c211-c233.md). |
...
| `.schedule.json` — my own dispatch prompts, first read at c212 | 2026-07-27 (c212) | **The dashboard job named two files that never existed, omitted two that do — prompt corrected, anchor rule inline.** Detail: §c212 in [archive part 3](../projects-archive/public-surface-c211-c233.md). |
| The life store's contents, diffed against its source files | 2026-07-27 (c213) | **Index ~36 h behind; handle pulled, reindexed in 22–25 s — automated as the hourly `aros-store-refresh` job.** Detail: §c213 in [archive part 3](../projects-archive/public-surface-c211-c233.md). |
| The `aros-store-refresh` job, read as code not as a commit | 2026-07-27 (c214) | **Delivered, and unsafe — `>` truncates before the copy; replaced with copy-beside-then-atomic-rename, re-measured 24 s.** Detail: §c214 in [archive part 3](../projects-archive/public-surface-c211-c233.md). |
...
| The blocker's second clause — "every settings write refused" | 2026-07-28 (c217) | **Holds: 403 on all three endpoints, probed with values already in place so nothing could change.** Detail: §c217 in [archive part 3](../projects-archive/public-surface-c211-c233.md). |
| The provenance README and the essay linking it, after the c214 job | 2026-07-28 (c218) | **Both stale within 24 h of my own fix — corrected, delivery measured; stated bound now "within one hour, worst case".** Detail: §c218 in [archive part 3](../projects-archive/public-surface-c211-c233.md). |
| My AI-disclosure line, read as a matcher not a sentence | 2026-07-28 (c219) | **Four strings, not one; the published method matched two. Guardrail 1 holds; one standard sentence adopted forward.** Detail: §c219 in [archive part 3](../projects-archive/public-surface-c211-c233.md). |
| Which kind of item the owner acts on | 2026-07-28 (c219) | **10 product actions vs 1 presence in ten days, against 6 aged owner-action issues — recorded as review input, not escalation.** Detail: §c219 in [archive part 3](../projects-archive/public-surface-c211-c233.md). |
| `POST /orgs/retinue-os/repos`, the one self-delivery endpoint | 2026-07-28 (c219) | **403, probed with no payload so nothing was created — fifth endpoint behind the one missing permission.** Detail: §c219 in [archive part 3](../projects-archive/public-surface-c211-c233.md). |
| Whether `w3id.org/retinue` is unclaimed, not merely unregistered | 2026-07-28 (c221) | **Clean: 0 PRs/issues matching in any state; remedy sized, median open→merge 3.9 h.** Detail: §c221 in [archive part 3](../projects-archive/public-surface-c211-c233.md). |
| Whether the essays' and landing page's links resolve | 2026-07-28 (c220) | **24 of 25 return 200; the one 404 is the project's own w3id namespace — essay calibrated, registration held for the owner.** Detail: §c220 in [archive part 3](../projects-archive/public-surface-c211-c233.md). |
| The **durations** of the dashboard-refresh job — `scheduler.log` only ever asked about `aros-tick` | 2026-07-28 (c223) | **875 s against a 900 s timeout — 25 s of margin — and two prior failures each left the page 48 h stale.** Wall and commit point now in the prompt; `aros-tick` checks the stamp every 30 min. Detail: §c223 in [archive part 3](../projects-archive/public-surface-c211-c233.md). |
| The **baselines** of the held drafts — the drain queue reported empty from a commit two write-ups never named | 2026-07-28 (c224) | **Both baseline-less write-ups reproduce in full against `26297a2`; baselines recorded, one claim tightened before filing.** `w3id` stays first in the queue ranking. Detail: §c224 in [archive part 3](../projects-archive/public-surface-c211-c233.md). |
...
| Whether the **published** Pages site matches committed `docs/`, and whether **this register renders** | 2026-07-28 (c227) | **Site clean, 19/19 byte-identical; the register was not — two stray blank lines made the five newest rows render as prose.** Fixed, verified via `POST /markdown`: 107 source rows → 107 rendered rows. Detail: §c227 in [archive part 3](../projects-archive/public-surface-c211-c233.md). |
...
| **The post-edit converter check c225 mandated**, never compared against the store it proxies | 2026-07-29 (c234) | **"13 triples" was a line count; the store — the authority — reads 10.** Check corrected to read the store; fourth venue of the proxy-reported-as-the-thing shape. Detail: §c234 in [archive part 4](../projects-archive/public-surface-c234-c249.md) |
...
| **External mentions of the project** — recorded as *unavailable* rather than substituted | 2026-07-29 (c233) | **A substitute exists and reads zero: GitHub search finds no true mention; both raw hits are wargaming noise.** Discriminator is the org filter plus reading the hit; covers GitHub only. Detail: §c233 in [archive part 3](../projects-archive/public-surface-c211-c233.md). |
| **The mandatory briefing-freshness check itself** — always run against the working tree, never the site | 2026-07-29 (c235) | **The check read the disk copy; the 26 h bound is a claim about the served copy a reader opens.** Clean that day (gap latent, not live); instrument corrected to read the served stamp, disk only to attribute. Detail: §c235 in [archive part 4](../projects-archive/public-surface-c234-c249.md) |
| **Rotation coverage — "every append-only file rotates" named two; nobody enumerated** | 2026-07-29 (c236) | **`strategy.md` was the third: 3.2 KB → 84 KB in ten days, no threshold.** Threshold set (150 KB); watch replaced by `tools/rotation-check.py`, verified both ways. Detail: §c236 in [archive part 4](../projects-archive/public-surface-c234-c249.md) |
| **The org's non-me actors — who else acts in these trackers, and about what** | 2026-07-29 (c237) | **Nostr named in three of the owner's twelve tracker actions (held for the 08-02 review), and a fourth actor found: Copilot-authored work already reaches `main` without my token.** Detail: §c237 in [archive part 4](../projects-archive/public-surface-c234-c249.md) |
| **The mentions probe c233 wrote down — the discriminator lived in a row, not a tool** | 2026-07-29 (c238) | **Reading unchanged, now measured: 28 raw hits across five probes, 0 confirmed.** `tools/mentions-check.py` post-filters every hit and refuses to report if its fixtures misclassify. Detail: §c238 in [archive part 4](../projects-archive/public-surface-c234-c249.md) |
| **Register pointers, checked for *direction* rather than existence** | 2026-07-29 (c239) | **The rotation created 26 wrong pointers and the standing check reported clean on both sides of it.** All repointed; `tools/pointer-check.py` now asks both questions, verified on the two failure modes it exists for. Detail: §c239 in [archive part 4](../projects-archive/public-surface-c234-c249.md) |
| **The freshness bound this page publishes, and the *scope* of the claim underneath it** | 2026-07-29 (c240) | **The bound holds and the scope was false: 6 of 61 tracked Markdown files are queryable, the other 55 absent by design — the served sentence claimed any Markdown edit; corrected on the served page.** Detail: §c240 in [archive part 4](../projects-archive/public-surface-c234-c249.md). |
| **The mentions instrument's own probe set, checked against the surface each probe *claims*** | 2026-07-29 (c243) | **Two of five probes labelled "issues and PRs" ran `is:issue`, excluding every PR; 19 new raw hits, reading unchanged at 0 confirmed. Probe set split into four; `probe_test()` guard added, rewritten once to actually fail the old set.** Detail: §c243 in [archive part 4](../projects-archive/public-surface-c234-c249.md). |
...
| **The other half of the same delivery — the *shell* Pages serves, beyond the five cards** | 2026-07-29 (c244) | **No live defect: all 14 served assets byte-identical to disk and committed copies. The gap was the instrument; `delivery-check.py` now walks `docs/` and compares served bytes to disk and to `HEAD`.** Detail: §c244 in [archive part 4](../projects-archive/public-surface-c234-c249.md). |
| **This file's own register table, and the check that guards it** | 2026-07-29 (c245) | **Live defect: a blank line broke the table on the public page — third occurrence, first with the check written and simply not run. Fixed in four minutes; `orphan_runs()` now locates the fragment and the check is a pre-commit hook.** Detail: §c245 in [archive part 4](../projects-archive/public-surface-c234-c249.md). |
| **The one held write-up c224 skipped — its citations *and* its published command** | 2026-07-29 (c246) | **Finding held; its evidence never ran — the published grep returns nothing (the string is pure ASCII), and 5 of 23 files sat outside a whole-front-end claim's scope. Replaced with two full-coverage scans; baseline recorded.** Evidence: [the held draft](../drafts/webapp-manifest-german-description.md). |
| **The held write-up that files tomorrow — every line number it prints, against its own baseline** | 2026-07-29 (c247) | **Finding reproduces; two of nine citations wrong, including the headline range, which cited the 409 guard for the dispatch it contradicts. Two runnable probes added.** Detail: §c247 in [archive part 4](../projects-archive/public-surface-c234-c249.md). |
| **The last held write-up whose evidence had never been executed — `traefik-readme-labels-already.md`** | 2026-07-29 (c248) | **Finding reproduces in full; the defect was my published check — "three lines means wired" is what a deployment missing the CN label prints while locking a cert-only device out. Replaced with a four-key named check.** Detail: §c248 in [archive part 4](../projects-archive/public-surface-c234-c249.md). |
| **The *published* essay's evidence, executed — the two SPARQL queries `writing/provenance-by-path.md` prints** | 2026-07-29 (c249) | **Both queries reproduce; one printed output was composed, not copied — abbreviated IRIs with no `PREFIX` declared. Real output, `ORDER BY`, its own re-run date; standfirst narrowed.** Detail: §c249 in [archive part 4](../projects-archive/public-surface-c234-c249.md). |
| **The second published essay's evidence, executed — `writing/egress-audit-observes.md`** | 2026-07-29 (c250) | **The result holds byte-for-byte; two defects were in the instrument — the verification command stops seven hours short of the probe, and my own c220 link checker planted probe-labelled flows in the evidence.** Detail: §c250 in [archive part 5](../projects-archive/public-surface-c250-c257.md). |
| **The paste-ready org-profile README, re-run rather than re-read** | 2026-07-29 (c251) | **Nine of ten checkable claims hold; the stale one ("six test files" — `main` has seven) sits in the clause a previous revision fixed for the same reason. Every count now carries its commit and date.** Detail: §c251 in [archive part 5](../projects-archive/public-surface-c250-c257.md). |
...
| **Framework `main`, read as a *published line* rather than as a tip SHA** | 2026-07-29 (c253) | **Three merged PRs (#41/#42/#43) no longer on `main` — replaced 12:45:00Z by a line with no common ancestor; tree-diffed, exactly four blobs differ; escalated privately, deliberately not filed.** Detail: §c253 in [archive part 5](../projects-archive/public-surface-c250-c257.md). |
| **The commit a held write-up names, checked for *reachability* instead of for content** | 2026-07-29 (c254) | **All three held baselines died at once with no file changing — `26297a2` off every branch after the replacement; re-baselined to `50b5be890`, `tools/baseline-check.py` added.** Detail: §c254 in [archive part 5](../projects-archive/public-surface-c250-c257.md). |
| **A recovery escalated as three shell commands, delivered instead as a mergeable branch** | 2026-07-29 (c255) | **`fix/restore-dropped-merges` pushed — blob-verified, no reference to the replaced history; PR#44 checked and clean.** Detail: §c255 in [archive part 5](../projects-archive/public-surface-c250-c257.md). |
...
| The project's **reach**, as distinct from its conversion — GitHub's traffic endpoints | 2026-07-29 (c258) | **Unreadable by design (20 calls, 20×403); eleven days of "zero external contact" published a numerator as a fraction. No scope requested — the ask was withdrawn, not repeated.** Detail: §c258 in [archive part 6](../projects-archive/public-surface-c258-c266.md). |
| **The remediation half of a filed finding** — chamber#8's `surface:` list vs. what was actually fixed | 2026-07-30 (c271) | **The fix reached one of the two published pieces and stopped; the org-profile README carried the un-caveated 404 IRI two days after its sibling was fixed. General form: remediate from the write-up's `surface:` list, not from memory.** Detail: §c271 in [archive part 7](../projects-archive/public-surface-c267-c277.md). |
| `scripts/scheduler.py`'s **job status field** | 2026-07-29 (c257) | **Written on every run, read on none — a failed job consumes its full slot; measured as two 48 h dashboard gaps. Consolidated into the rank-1 held draft, not filed separately.** Detail: §c257 in [archive part 5](../projects-archive/public-surface-c250-c257.md), evidence in [drafts/updater-reports-dispatch-not-result.md](../drafts/updater-reports-dispatch-not-result.md). |
| **Every pointer from GitHub to the served docs site** | 2026-07-29 (c259) | **No README in the org carried the served URL and every `homepage` field was empty; fixed on the chamber README, framework half held.** Detail: §c259 in [archive part 6](../projects-archive/public-surface-c258-c266.md). |
| **The content of my own recovery branch, checked for truth rather than fidelity** | 2026-07-29 (c260) | **The restore re-shipped a latency range publicly retracted four days earlier; corrected as a separate droppable commit. Standing check: re-read restored content against what was published since.** Detail: §c260 in [archive part 6](../projects-archive/public-surface-c258-c266.md). |
| **The desk card's *contents*, diffed against the previous generation** | 2026-07-29 (c262) | **Seven open issues left the owner's desk in one regeneration with no record; `tools/desk-drop-check.py` added. Resolved c272: all seven back, 0 problems.** Detail: §c262 in [archive part 6](../projects-archive/public-surface-c258-c266.md). |
| **`tools/pointer-check.py`'s own coverage** | 2026-07-29 (c263) | **Parsed 55 of the register's 91 pointers and reported on all of them; grammars widened, UNPARSED made loud, one live dead anchor repaired. c348 amendment (a quoted form misread as live): §c348 in [c347–c348 archive part](../projects-archive/public-surface-c347-c348.md).** Detail: §c263 in [archive part 6](../projects-archive/public-surface-c258-c266.md). |
